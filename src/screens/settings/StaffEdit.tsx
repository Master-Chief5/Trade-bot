...
export function StaffEdit({ user }: { user: StaffUser }) {
  ...
  if (!existing && !canManage) return <Navigate to="/settings/staff" replace />;
  ...
  const save = () => {
    ...
    if (existing && !canManage) {
      actions.updateStaff(existing.id, { floorIds }, user);
      toast('Floors saved');
      ...
      <Card pad>
        <div className="stack">
          {canManage ? (
            ...
            <p className="muted small">You can change which floors {existing?.name} covers. A dean handles names, PINs and roles.</p>
          )}
          {isDean && (
          ...
          {canManage && <TextInput label={existing ? 'New PIN (leave blank to keep)' : 'PIN'} inputMode="numeric" pattern="[0-9]*" maxLength={6} value={pin} onChange={(e) => setPin(e.target.value.replace(/\D/g, ''))} help="4 to 6 digits. Tell them in person." />}
          {canAssign && role !== 'dean' && (
            <div className="field">
